...
#define TRAFFIC_LIGHT_GREEN D6    // Green LED control (GPIO12) - moved from D0
...
#define SMOOTH_SAMPLES 3           // Reduced for real-time response (was 10)
...
float co2_ppm = 0;
float nh3_ppm = 0;   // Ammonia concentration
float benzene_ppm = 0; // Benzene concentration
...
float calculateNH3(float rs) {
  // Calculate Ammonia concentration
  float ratio = rs / MQ135_R0;
  if (ratio <= 0.01) ratio = 0.01;
  if (ratio >= 10.0) ratio = 10.0;
  
  // NH3 calculation for MQ135
  float ppm = 100.0 * pow(ratio, -2.0);
  return constrain(ppm, 0, 100);
}

float calculateBenzene(float rs) {
  // Calculate Benzene concentration
  float ratio = rs / MQ135_R0;
  if (ratio <= 0.01) ratio = 0.01;
  if (ratio >= 10.0) ratio = 10.0;
  
  // Benzene calculation for MQ135
  float ppm = 50.0 * pow(ratio, -1.8);
  return constrain(ppm, 0, 50);
}

float mapToAQI(float co2_ppm, float nh3_ppm, float benzene_ppm) {
  // Comprehensive AQI calculation considering multiple pollutants
  
  // CO2 contribution (0-100 points)
  float co2_score = 0;
  if (co2_ppm <= 400) co2_score = 0;
  else if (co2_ppm <= 600) co2_score = map(co2_ppm, 400, 600, 1, 33);
  else if (co2_ppm <= 1000) co2_score = map(co2_ppm, 600, 1000, 34, 66);
  else if (co2_ppm <= 1500) co2_score = map(co2_ppm, 1000, 1500, 67, 100);
  else co2_score = 100;
  
  // NH3 contribution (0-50 points)
  float nh3_score = 0;
  if (nh3_ppm <= 5) nh3_score = 0;
  else if (nh3_ppm <= 25) nh3_score = map(nh3_ppm, 5, 25, 1, 50);
  else nh3_score = 50;
  
  // Benzene contribution (0-50 points)
  float benzene_score = 0;
  if (benzene_ppm <= 1) benzene_score = 0;
  else if (benzene_ppm <= 10) benzene_score = map(benzene_ppm, 1, 10, 1, 50);
  else benzene_score = 50;
  
  // Calculate total AQI (0-200 scale)
  float total_aqi = co2_score + nh3_score + benzene_score;
  
  return constrain(total_aqi, 0, 200);
}

String getAQICategory(float aqi) {
  if (aqi <= 25) return "EXCELLENT";
  else if (aqi <= 50) return "GOOD";
  else if (aqi <= 75) return "MODERATE";
  else if (aqi <= 100) return "UNHEALTHY";
  else if (aqi <= 150) return "VERY BAD";
...
void handleAlerts() {
  ...
  // Determine alert level using comprehensive AQI and sensors
...
void readSensors() {
  ...
  // Enhanced MQ135 calculations for multiple pollutants
  if(!mq135_disconnected) {
    ...
    nh3_ppm = calculateNH3(rs_135);
    benzene_ppm = calculateBenzene(rs_135);
    calculated_aqi = mapToAQI(co2_ppm, nh3_ppm, benzene_ppm);
  } else {
...
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="version">v1.0 by Group: 7 (Section: 2)</div>
        </div>
        ...
        <div class="grid">
            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-icon">🧪</div>
                    <div class="card-title">NH₃ Level</div>
                </div>
                <div class="metric-value" id="nh3">--<span class="metric-unit">ppm</span></div>
                <div class="metric-label">Ammonia Concentration<span class="badge">MQ135</span></div>
            </div>
            
            <div class="card">
                <div class="card-header">
                    <div class="card-icon">⚗️</div>
                    <div class="card-title">Benzene Level</div>
                </div>
                <div class="metric-value" id="benzene">--<span class="metric-unit">ppm</span></div>
                <div class="metric-label">Benzene Concentration<span class="badge">MQ135</span></div>
            </div>
    ...
    <script>
            ...
            fetch('/api/data')
                ...
                .then(function(data) {
                    ...
                    document.getElementById('nh3').innerHTML = 
                        data.nh3.toFixed(1) + '<span class="metric-unit">ppm</span>';
                    
                    document.getElementById('benzene').innerHTML = 
                        data.benzene.toFixed(2) + '<span class="metric-unit">ppm</span>';
        ...
        updateData();
        setInterval(updateData, 1000); // Update every second for real-time data
    </script>
</body>
</html>
)rawliteral";
...
void handleAPI() {
  ...
  int nh3_int = (int)(nh3_ppm * 10);
  int benzene_int = (int)(benzene_ppm * 100);
  ...
  json += "\"nh3\":" + String(nh3_int / 10.0, 1) + ",";
  json += "\"benzene\":" + String(benzene_int / 100.0, 2) + ",";
...
void setup() {
  ...
  Serial.println("Author: Group: 7 (Section: 2)");
  Serial.println("MQ135: Comprehensive air quality sensor (5V operation)");
  Serial.println("  - CO2, NH3, Benzene detection");
  Serial.println("  - Enhanced AQI calculation (0-200 scale)");
  Serial.println("Traffic Light: D6=Green, D8=Yellow, D3=Red");
...
void loop() {
  ...
  // Print to serial with comprehensive MQ135 values
  Serial.printf("Mode: %d | Temp: %.1f°C | Humidity: %.1f%% | AQI: %.0f | CO2: %.0f ppm | NH3: %.1f ppm | Benzene: %.2f ppm | MQ135: %d | Alert: %d\n", 
                currentMode, temp_smoothed, humidity_smoothed, calculated_aqi, co2_ppm, nh3_ppm, benzene_ppm, mq135_analog, currentAlert);
  
  // Debug: Print raw sensor values every 10 seconds
  static unsigned long lastDebugTime = 0;
  if (millis() - lastDebugTime > 10000) {
    Serial.println("=== MQ135 DEBUG INFO ===");
    Serial.printf("Raw Analog: %d | Voltage: %.2fV | Digital: %s\n", 
                  mq135_analog, 
                  ((float)mq135_analog / 1023.0) * 5.0,
                  mq135_digital == LOW ? "GAS DETECTED" : "NO GAS");
    Serial.printf("Smoothed: %.1f | R0: %.1f | RL: %.1f\n", 
                  mq135_smoothed, MQ135_R0, MQ135_RL);
    Serial.println("========================");
